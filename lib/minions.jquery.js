...
JQueryMinions.prototype.aHTMLContainer = function(options) {
	...
	if (typeof this.options.id === 'string') {
		...
		this.DOMelement.id = this.makeId(16);
	}
...
JQueryMinions.prototype.preLoadImage = function(url,callback) {
	...
	jQuery(newImg).attr('id', this.makeId(16));
...
JQueryMinions.prototype.addAudio = function(Id, parentDomSelector, sources, options) {
	...
	var _default__Id = 'audio__' + this.makeId(16);
...
// simple extension function for jQuery to set a DOM element's zoomLevel for all major browsers
(function(jQuery) {
...
;( function() {
	...
	for (var i in JQueryMinions.prototype) {
		Minions.prototype[i] = JQueryMinions.prototype[i];
	}
} ) ();
