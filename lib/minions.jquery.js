 ...
 *
 *	------------------------------------------------------------------*/
JQueryMinions.prototype.isInBox = function(selector, x1, y1, x2, y2) {
    var elements = [];
    jQuery(selector).each(function() {
        var $this = jQuery(this);
        var offset = $this.offset();
        var x = Math.round(parseInt(offset.left));
        var y = Math.round(parseInt(offset.top));
        var w = Math.round(parseInt($this.width()));
        var h = Math.round(parseInt($this.height()));
        if (x >= x1 
            && y >= y1 
            && x + w <= x2 
            && y + h <= y2) {
...
JQueryMinions.prototype.drawHelperLine = function(x, y, z, color, parent) {
  ...
  jQuery(helperLine).hover(function(e) { jQuery(helperLine).css('border-top', '3px solid ' + borderColor); }, function(e) { jQuery(helperLine).css('border-top', '1px solid ' + borderColor); });
  return helperLine;
};
