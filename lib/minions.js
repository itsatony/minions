...
Minions.prototype.exceptionHandler = function(err, data) {
	...
	console.log('[--ERROR--]');
	console.log(err);
	if (typeof vws === 'object') {
		var util = require('util');
		var isNodeError = util.isError(err);
		if (isNodeError && err.stack !== 'undefined') {
		...
		vws.errors.push(logError);
		if (vws.errors.length > 100) vws.errors.shift();
	} else if (typeof vwc === 'object') {
		var logError = {
			err: err,
			stack: (typeof err === 'object' && typeof err.stack !== 'undefined') ? err.stack : 'no stack trace',
			timestamp: Date.now()
		...
		if (vwc.errors.length > 100) vwc.errors.shift();
	}
	return true;
};
