 ...
 *		target - {object} the extended target object
 * --------------------------------------------------------------------------- */
Minions.prototype.extendPrototypes = function(doNotOverwrite, target, sourceArray) {
	if (typeof doNotOverwrite !== 'boolean') doNotOverwrite = false;	
...
Minions.prototype.randomString = function(length, numbers, alphabet, timestamp) {		
	...
	var both = new Array('1','2','3','4','5','6','7','8','9','0', 'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z');
	if (numbers === true && alphabet === false) {
		var idchars = numbers;
	} else if (numbers === true && alphabet === true) {
		var idchars = both;
	} else if (numbers === false && alphabet === true) {
		var idchars = alphabet;
	}
...
Minions.prototype.exceptionHandler = function(err, data) {
	...
	if (typeof VisualWebServer === 'object') {
		...
		if (VisualWebServer.config.debugging > 0) log.add('err[' + err + '] data[' + data + ']', 'error', 'exceptionHandler');
	} elsa if (typeof VisualWebClient === 'object') {
...
Minions.prototype.tagsToString = function(tags) {
	...
	if (this.isArray(tags)) return tags.toString();
	var tagString = '';
	for (var i in tags) {
		tagString += tags[i] + ', ';
	}
 ...
 * --------------------------------------------------------------------------- */
VisualWebClient.funcs.getFirstObjectAttribute = function(obj) {
	var first;
	for (var i in obj) {
 ...
 * --------------------------------------------------------------------------- */
VisualWebClient.funcs.getNextObjectAttribute = function(obj, attName) {
	var first = false;
 ...
 *
 *	parameters:
 *		anArray - {object} the object to iterate over 
 *
 *	returns:
 *		keys - {array} the list of found keys
 * --------------------------------------------------------------------------- */
Minions.prototype.array_keys = function(anObject) {
	var keys = [];
...
Minions.prototype.runOncePer = function(callback, delay, resetOnRecall, isInterval, scope, callbackParameters) {
	...
	this.callbackParameters = (this.isArray(callbackParameters)) ? callbackParameters : [];
	return this;
};
/* -----------------------------------------------------------------
...
Minions.prototype.workWithObjectAsync = function(sourceObject, functionToRun, parametersToGive, callbackForTheObject) {
	...
	functionToRun.apply(this, parametersToGive);
	return true;
};
