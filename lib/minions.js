 ...
 * --------------------------------------------------------------------------- */
Minions.prototype.randomString = function(length, numbers, alphabetLowerCase, timestamp, alphabetUpperCase) {   
  // version TW 15.04.2013 18.00
  var id = '';
  ...
  if (alphabetLowerCase === false && alphabetUpperCase === false && numbers === false) {
    numbers = true;
...
Minions.prototype.objectFilter.prototype.run = function(sourceObject, additionalData) {
	...
	for (var key in sourceObject) {
		...
		else if (typeof sourceObject[key] === 'function') resultObject[key] = '{function}';
    ...
    // ??? what is the next line good for?
		...
		resultObject[newAttName] = this.config.attributesToAdd[newAttName](sourceObject, additionalData);
	}
	return resultObject;
};
...
Minions.prototype.delayedFor = function(aArray, delay, callPerItem, callAfter, logTimings) {
	if (logTimings) this.startTime = new Date().getTime();
	this.array = aArray;
	this.counter = -1;
	...
	this.end = function() {
		if (logTimings) console.log('TIME NEEDED : ' + (new Date().getTime() - thisDelayedFor.startTime) );
