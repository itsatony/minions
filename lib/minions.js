...
function clone(a) {
	...
	if (a instanceof Array) {
		b = [];
		for (var f = 0; f < a.length; f++) b.push(a[f]);
	} else {
		for (var f in a) b.valueOf()[f] = a.valueOf()[f];
	}
	return b;
};

function copy(a) {
	if (typeof a === 'number') return Number(a.valueOf());
	if (typeof a === 'string') return String(a.valueOf());
	if (typeof a === 'object') return clone(a.valueOf());
	if (typeof a === 'function') return a.valueOf();
	if (typeof a === 'boolean') return Boolean(a.valueOf());
};
