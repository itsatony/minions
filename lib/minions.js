...
Minions.prototype.objectFilter.prototype.run = function(sourceObject) {
	...
	for (var key in sourceObject) {
		...
		if (typeof filter[key] === 'undefined' && this.config.returnUnfilteredAttributes === true) {
			...
			resultObject[key] = filter[key](sourceObject[key]);
		}
	}
 ...
 *		key and if there is an undefined key on the way, it givs an 'undefined'
 *		and not an error
 *
 *	parameters:
 *		object - {object} the aliases to assign to the func.
 *		key - {string} the function to assign to the aslias(es)
 *
 *	returns:
...
Minions.prototype.keyTypeof = function (object, key) {
	...
	var keyObject = object[objectArray.shift()];

	if (typeof keyObject === 'undefined') {
		return 'undefined';
	}
	if (objectArray.length === 0) {
		return typeof keyObject;
	}
	
	return this.keyTypeof(keyObject, objectArray.join('.'));
}
